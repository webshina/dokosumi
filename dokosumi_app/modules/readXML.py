...
# ファイル一覧を再帰的に取得
xml_list = glob.glob('D:\programs\Python\Dokosumi\data\地価公示\L**\L*.xml', recursive=True)
# xml_list = ['D:\programs\Python\Dokosumi\data\地価公示\L01-12_08_GML\L01-12_08.xml']
print(xml_list)
...
for xml_file in xml_list:
    ...
    # ksj:LandPrice以下を取得
    pointNodes = root.findall('ksj:LandPrice', ns)
    ...
    for itemNode in pointNodes:
        ...
        # idを取得
        if itemNode.find('ksj:position', ns) is not None:
            id = itemNode.find('ksj:position', ns).attrib['{' + ns['xlink'] + '}' + "href"].replace('#','')
            # SCOREを取得
            score = itemNode.find('ksj:postedLandPrice', ns).text.strip()
